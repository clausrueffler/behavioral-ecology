...
NOTATION ALERT: In this exercise, mutant strategies are named $x'$ in the running text and $x.m$ in the R-code. Similarly, resident strategies are named $x$ in the running text and $x.r$ in the R-code. Thus, $x' = x.m$ and $x = x.r$. The reason for the different naming in the R-code is that $x'$ is not a permitted name in R-code.
...
Note, that yellow areas indicate combinations of mutants and residents such that the mutant has a negative invasion fitness (it cannot invade), while red corresponds to combinations of mutants and residents such that the mutant has a positive invasion fitness (it can invade). As we can see, only mutants with $x' < x$ can invade, confirming our above analytical result.

We can arrive at the same conclusion by looking at the partial derivative of $W(x', x)$ with respect to $x'$ when evaluated at $x' = x$, which equals
...
W(x', x) & = r w(x', x', x) + (1 - r) w(x', x, x) \\
        & = (1 - x')\left(r (S + B x') + (1 - r) (S + B x)\right) \\
        & = (1 - x') \left((S + B x) + r B (x' - x)\right),\\
        \\
W(x, x) & = r w(x, x, x) + (1 - r) w(x, x, x) \\
        & = w(x, x, x)\\
        & = (1 - x)(S + B x),
...
\begin{align}
s(x', x) & = W(x', x) - W(x, x)\\
         & = (1 - x') \left((S + B x) + r B (x' - x)\right) - (1 - x)(S + B x)\\
         & = -(S + B x) (x' - x) + r (1 - x') B\\
         & = (x' - x) \left(-(S + B x) + r (1 - x') B\right)
\end{align}
$$         
...
\end{align}
$$
indicating that selection favors mutants with increased propensity to self-sacrifice if and only if $rb>c$, that is, if Hamilton's rule is fulfilled.
...
From this graph, we can read that $x^* = 0.33$ is an ESS. If the resident strategy is equal to this value, then  $s(x', x^*) < 0$ for all $x' \neq x^*$.
...
Importantly, there is NO *evolutionary branching point* in this model. Thus, such a polymorphism cannot emerge by gradual evolution of $x$. It can only emerge through a mutation with a relatively large mutational effect on $x$. However, one can show that for areas of coexistence that are not connected to an evolutionary branching point, further evolution of the two coexisting strategies results in the convergence of the two strategy values and ultimately in the loss of one of them. Evolution will then result in the ESS $x^*$.
