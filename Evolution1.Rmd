...
In this last plot the red area corresponds to combinations of heights *h'* and *h* where neither strategy can invade the other. In other words: whoever was first wins. In contrast, the blue areas correspond to combinations of heights where either the mutant or the resident wins. These results show that in the current model it is not possible for two plant species with different heights to coexist.
...
1. Evaluate all the above code. Understand what the results mean. Try to understand the code as much as possible (but note that code is not be relevant for the exam).
