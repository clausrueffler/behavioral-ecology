...
filled.contour(t(PIP), levels = c(-1, 0, 1), main = "PIP", las = 2,
               xlab = expression("height " *h[1]),
               ylab = expression("height " *h[2]))
...
filled.contour(t(PIP.mirrored), levels = c(-1, 0, 1), main = "PIP.mirrored", las = 2,
               xlab = expression("height " *h[2]),
               ylab = expression("height " *h[1]))
...
curve(P.L.flex(x, h.2 = 0.5), from = 0, to = 1,
      xlab = expression("height "* h[1]),
      ylab = expression("minimum photosynthesis, " * P[L](h[1], h[2] * "=0.5")))
```
...
curve(g.2(x, h.2 = 0.5), from = 0, to = 1, col = "blue",
      xlab = expression("plant height, " * h[1]), ylim = c(0,1),
      ylab = expression("photosythesis/leaf, " * g(h[1], h[2] * "= 0.5")))
curve(g(x, h.2 = 0.5), from = 0, to = 1, col = "orange", add = TRUE)

legend("topleft", lty =1, col = c("blue", "orange"),
       legend = c(expression(P[L] * " fixed "), expression(P[L] * " variable ")))
```
...
par(pty="s")
filled.contour(t(PIP.mirrored), levels = c(-1, 0, 1), las = 2, main = "PIP.mirrored", xlab = expression("height " *h[2]), ylab = expression("height " *h[1]))
