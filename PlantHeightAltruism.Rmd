...
When comparing with the results from the first tab, we can see that kin selection reduces the ESS plant height from approximately 0.62 to approximately 0.5.
...
\begin{align}
\frac{\partial W(h', h)}{\partial h'}\bigg|_{h' = h} 
  & = \frac{\partial}{\partial h'}(f(h')g(h', h)) + r \frac{\partial}{\partial h}(g(h', h)).
\end{align}
$$
...
-->
The first term describes the effect of increasing the strategy value $h'$ of the actor on the payoff of the actor itself. This term is zero when $h'=h^*$, where $h^*$ is the evolutionary stable strategy value in the absence of kin competition, negative when $h'>h^*$, and positive when $h^*<h'$. Thus, for a plant to decrease its height to a value lower than $h^*$ can be considered an act of altruism.
