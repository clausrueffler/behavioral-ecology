...
Next we reproduce the payoff matrix shown in Table 6.1 in Kokko (2007). First, we define the functions for these payoffs.

```{r}
w.h.m.h.r <- function(h.m, h.r) { # payoff for h'
  f(h.m) * g(h.m, h.r)
}

w.h.r.h.m <- function(h.r, h.m) { # payoff for h
  f(h.r) * g(h.r, h.m)
}
```
...
```{r}
round(outer(c(0,1/3,2/3,1), c(0,1/3,2/3,1), w.h.m.h.r), digits = 3)
```

Payoff to plant with height $h$ (second number in the matrix in Kokko).

```{r}
round(outer(c(0,1/3,2/3,1), c(0,1/3,2/3,1), w.h.r.h.m), digits = 3)
```
...
```{r}
curve(w.h.m.h.r(x, h.r), from = 0, to = 1, xlab = expression("height h'"), ylab = expression("payoff w(h', h = 0.5)"))
```
...
best.response <- function(h.r) {
  optimize(w.h.m.h.r, interval=c(0, 1), maximum = TRUE, h.r)$maximum
}

best.response(h.r = 0.5)
```

In conclusion, if plant 2 plays strategy $h = 0.5$, then plant 1 maximizes its pay-off by playing $h' = 0.58$. To obtain the best response curve, perform the computation above for all values of $h$ between 0 and 1. After that we repeat the same procedure from the perspective plant 2.
